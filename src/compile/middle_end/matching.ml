...
let rec matching e =
  ...
  (* | E_letIn(p,e1,e2) when combinatorial e1 ->
      matching @@ subst_p_e p e1 e2*)
  (* | E_letIn(p,e1,e2) ->
     (match p,matching e1 with
      | P_tuple ps, E_tuple vs ->
         matching @@
         List.fold_right2 (fun p v acc -> E_letIn(p,v,acc)) ps vs e2
      | P_var x, E_var y ->
         matching @@
         subst_e x (E_var y) e2
      | P_var x, ((E_tuple vs) as v) -> (* tuple are in normal form : (x,y) *)
         matching @@
         subst_e x v e2
      | P_var x, e1 ->
         E_letIn(P_var x,e1,matching e2)
      | (P_tuple ps) as p,e1 ->
         let len = List.length ps in
         let z = Ast.gensym () in
         let name i = z^"_"^string_of_int i in
         assert (List.for_all (fun P_var _ -> true | _-> false) ps);
         E_letIn(P_var z,e1,
           List.fold_right (fun p (acc,i) -> E_letIn(P_var (name i),v,acc),i+1) ps vs e2
          matching @@
                            E_letIn(P_var z,pat2exp p,e2))
      | P_unit,_ -> assert false)*)
