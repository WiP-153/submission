...
let pp_component fmt ~name ~state_var ~argument ~result ~compute ~rdy typing_env infos (ts,s) =
  ...
  Hashtbl.iter (fun x t ->
      if x <> result && x <> argument then
        if not (List.mem x vars) then
          fprintf fmt "signal %s : value(0 to %d);@," x (size_ty t-1)
    ) typing_env;
  ...
  Hashtbl.iter (fun x t ->
      if x <> result && x <> argument then
        if (List.mem x vars) then
          fprintf fmt "variable %s : value(0 to %d);@," x (size_ty t-1)
    ) typing_env;
